    ...
    with open(os.devnull, 'w') as f, redirect_stdout(f), redirect_stderr(f):
      ...
      for i in [0, 1]:
        ...
        if model_nm in pretrained:
          ...
          stardist_models_dir_path = os.path.join(importlib.resources.files('napari_bruce'), 'stardist_models')
...
class LoadWorker(BaseWorker):
  ...
  def compute(self):
    ...
    # Define output paths 
    out_dir_path = os.path.join(Path(self.config['out_dir_path']).expanduser(), Path(self.path).stem)
    ...
    metadata = {**raw_metadata, 'channels': dict(list(raw_metadata['channels'].items())[:2])}
  ...
  def compute(self):
    ...
    for k, v in self.ch_names.items():
      ...
      if self.do_predict:
        ...
        submsks_area_um2 = {k1:v1*px_area_um2 for k1, v1 in submsks_area.items()}
      ...
      if self.do_predict:
        ...
        filt_msk_changed[v] = False if np.array_equal(filt_msk, prev_filt_msk) else True
...
class OverlapWorker(BaseWorker):
  ...
  def compute(self):
    ...
    def rgb_from_config(key):
      ...
      return tuple(int(x * 255) for x in to_rgba(self.config['elements'][key]['color'])[:3])
...
class PluginManager(QWidget):
  ...
  def start_worker_thread(
    ...
    if existing_thread and existing_thread.isRunning():
        ...
        if not allow_if_running:
          ...
          print(f'[start_worker_thread] {worker_class.__name__} not started: {thread_attr_name} still running.')
    ...
    worker.sig_error.connect(lambda msg, worker=worker_class.__name__: self.on_worker_error(worker, msg))
  ...
  def init_viewer_layers(self):
    ...
    with self.viewer.layers.events.blocker():
      ...
      for i in [0, 1]:
        ...
        self.layers[i]['image'] = self.viewer.add_image(image_array, 
                                                        name=f'ch{i} normalized image',
                                                        visible=False)
      ...
      for i in [0, 1]:
        ...
        self.layers[i]['shapes'] = self.viewer.add_shapes(data=[],
                                                          name=f'ch{i} - add',
                                                          shape_type='path',
      ...
      self.layers['merge'] = self.viewer.add_image(merge_array, 
                                                   name='Merge + ROIs',
                                                   rgb=True,
                                                   visible=False)
  ...
  def update_viewer_from_workflow(self):
    ...
    if workflow_state in {WorkflowState.SELECT_FILE, WorkflowState.LOAD_IMAGE, WorkflowState.LOADING_IMAGE, WorkflowState.LOADING_IMAGE_FOR_PREDICT_ROI}:
      
      viewer_state = ViewerState.NO_IMAGE
      
    elif workflow_state in {WorkflowState.IMAGE_LOADED_FOR_PREDICT_ROI, WorkflowState.PREDICT_ROI, WorkflowState.PREDICTING_ROI}:
    ...
    elif workflow_state in {WorkflowState.APPLYING_EDITS, WorkflowState.OVERLAP_ROI, WorkflowState.OVERLAPPING_ROI, WorkflowState.OVERLAP_FILTER_OR_SAVE, WorkflowState.UPDATE_OVERLAP_FILTER_OR_SAVE, WorkflowState.CLEARED}:
  ...
  def update_ui_from_workflow(self):
    ...
    elif workflow_state in {WorkflowState.LOADING_IMAGE, WorkflowState.LOADING_IMAGE_FOR_PREDICT_ROI}:
    ...
    if workflow_state in {WorkflowState.LOADING_IMAGE, WorkflowState.LOADING_IMAGE_FOR_PREDICT_ROI, WorkflowState.PREDICTING_ROI, 
                          WorkflowState.APPLYING_EDITS, WorkflowState.OVERLAPPING_ROI}:
  ...
  def set_predict_roi_ui(self, loaded_for_predict=False):
    ...
    self.ui.box_min_area_ch0 = ParamValueBox(label=f'min area (\u00B5m\u00B2) {self.workflow.ch_names[0]}', 
                                          default=self.config['channels'][0]['min_area_um2'],
                                          min_val=0.0, 
                                          max_val=10000.0)    
    self.ui.box_min_area_ch0.valueChanged.connect(lambda x: self.on_min_area_changed(0, x))
      
    self.ui.box_min_area_ch1 = ParamValueBox(label=f'min area (\u00B5m\u00B2) {self.workflow.ch_names[1]}', 
                                          default=self.config['channels'][1]['min_area_um2'],
                                          min_val=0.0, 
                                          max_val=10000.0)
    self.ui.box_min_area_ch1.valueChanged.connect(lambda x: self.on_min_area_changed(1, x))
    ...
    if loaded_for_predict:
      ...
      for i in [self.ui.box_min_area_ch0,  
                self.ui.box_min_area_ch1]:
  ...
  def set_overlap_roi_ui(self):
    ...
    self.ui.box_min_pct_ovl_ch0_by_ch1 = ParamValueBox(label=f'min % overlap {self.workflow.ch_names[0]} / {self.workflow.ch_names[1]}', 
                                                    default=self.config['min_pct_ovl_ch0_by_ch1'],
                                                    min_val=0.0, 
                                                    max_val=100.0)
    self.ui.box_min_pct_ovl_ch0_by_ch1.valueChanged.connect(lambda x: self.on_min_pct_ovl_changed('min_pct_ovl_ch0_by_ch1', x))
    
    self.ui.box_min_pct_ovl_ch1_by_ch0 = ParamValueBox(label=f'min % overlap {self.workflow.ch_names[1]} / {self.workflow.ch_names[0]}', 
                                                    default=self.config['min_pct_ovl_ch1_by_ch0'],
                                                    min_val=0.0, 
                                                    max_val=100.0)
    self.ui.box_min_pct_ovl_ch1_by_ch0.valueChanged.connect(lambda x: self.on_min_pct_ovl_changed('min_pct_ovl_ch1_by_ch0', x))
  ...
  def set_overlap_filter_or_save_ui(self, build_ui=True):
    ...
    if build_ui:
      ...
      self.ui.box_elem_ch0pos_ch1neg = ElementConfigBox(label=f'{self.workflow.ch_names[0]}{plus} / {self.workflow.ch_names[1]}{minus}')
      
      self.ui.box_elem_ch0pos_ch1neg.valueChanged.connect(lambda x: self.on_elem_params_changed('ch0-pos/ch1-neg', x))
      
      self.ui.box_elem_ch0pos_ch1pos = ElementConfigBox(label=f'{self.workflow.ch_names[0]}{plus} / {self.workflow.ch_names[1]}{plus}')
      
      self.ui.box_elem_ch0pos_ch1pos.valueChanged.connect(lambda x: self.on_elem_params_changed('ch0-pos/ch1-pos', x))
      
      self.ui.box_elem_ch0pos_ch1amb = ElementConfigBox(label=f'{self.workflow.ch_names[0]}{plus} / {self.workflow.ch_names[1]}-amb')
      
      self.ui.box_elem_ch0pos_ch1amb.valueChanged.connect(lambda x: self.on_elem_params_changed('ch0-pos/ch1-amb', x))
      
      self.ui.box_elem_ch1pos_ch0neg = ElementConfigBox(label=f'{self.workflow.ch_names[1]}{plus} / {self.workflow.ch_names[0]}{minus}')
      
      self.ui.box_elem_ch1pos_ch0neg.valueChanged.connect(lambda x: self.on_elem_params_changed('ch1-pos/ch0-neg', x))
      
      self.ui.box_elem_ch1pos_ch0amb = ElementConfigBox(label=f'{self.workflow.ch_names[1]}{plus} / {self.workflow.ch_names[0]}-amb')
      
      self.ui.box_elem_ch1pos_ch0amb.valueChanged.connect(lambda x: self.on_elem_params_changed('ch1-pos/ch0-amb', x))
    ...
    for i in [self.ui.box_min_pct_ovl_ch0_by_ch1,
              self.ui.box_min_pct_ovl_ch1_by_ch0, 
              self.ui.btn_overlap_filter,
              self.ui.box_elem_ch0pos_ch1neg,
              self.ui.box_elem_ch0pos_ch1pos,
  ...
  def update_viewer_data_on_load_finished(self):
    ...
    with self.viewer.layers.events.blocker():  
      ...
      self.layers['merge'].data = np.zeros(self.workflow.data[self.workflow.ch_names[0]]['norm_img'].shape+(3,),
                                           dtype=np.uint8)
  ...
  def update_viewer_data_on_apply_edits_finished(self):
    ...
    with self.viewer.layers.events.blocker():
      ...
      self.layers['merge'].data = self.workflow.data['merge']['merge_norm_img_rois']
  ...
  def start_load_thread(self):
    ...
    self.start_worker_thread(worker_class=LoadWorker, 
  ...
  def on_img_loaded_and_predict_clicked(self):
    ...
    self.set_message('Predicting ROIs...', 
                     MessageLevel.WORK)
  ...
  def start_predict_filter_thread(self, do_predict=True):
    ...
    self.start_worker_thread(worker_class=PredictFilterWorker, 
                             worker_args=(self.workflow.data, self.workflow.metadata, self.config, self.workflow.ch_names, self.models, do_predict), 
  ...
  def start_overlap_thread(self):
    ...
    self.start_worker_thread(worker_class=OverlapWorker, 
  ...
  def on_overlap_finished(self, worker_output: dict):
    ...
    if self.state.workflow_state == WorkflowState.OVERLAPPING_ROI:
      ...
      self.set_workflow_state(state=WorkflowState.UPDATE_OVERLAP_FILTER_OR_SAVE, force=True)
  ...
  def on_save_clicked(self):
    ...
    workflow.pickle_data(data=self.workflow.data, filename=os.path.join(out_dir_path, 'data.pkl'))
    
    workflow.pickle_data(data=self.workflow.metadata, filename=os.path.join(out_dir_path, 'metadata.pkl'))
  ...
  def update_elem_boxes(self):
    ...
    box = [self.ui.box_elem_ch0pos_ch1neg,
           self.ui.box_elem_ch0pos_ch1pos,
           self.ui.box_elem_ch0pos_ch1amb,
           self.ui.box_elem_ch1pos_ch0neg,
           self.ui.box_elem_ch1pos_ch0amb]
  ...
  def read_elem_boxes(self):
    ...
    for i, j, k in zip(primary_channel_id, secondary_channel_status, box):
      ...
      self.workflow.data[self.workflow.ch_names[i]]['summary'][f'{j}_collect'] = n_collect
      
      self.workflow.data[self.workflow.ch_names[i]]['summary'][f'{j}_laser_function'] = laser_function
      
      self.workflow.data[self.workflow.ch_names[i]]['summary'][f'{j}_tube_id'] = tube_id
    
    self.workflow.data[self.workflow.ch_names[0]]['summary'] = {k:self.workflow.data[self.workflow.ch_names[0]]['summary'][k]
    ...
    self.workflow.data[self.workflow.ch_names[1]]['summary'] = {k:self.workflow.data[self.workflow.ch_names[1]]['summary'][k]
                                              for k in ['total', 'neg', 'pos', 'amb', 
