...
def robust_normalization(
    img: np.ndarray, low_pct: float = 0.05, high_pct: float = 99.95
) -> np.ndarray:
    ...
    output = cv2.normalize(
        np.clip(img, low, high), None, 0, 255, cv2.NORM_MINMAX
    ).astype(np.uint8)
...
def msk_to_cnts(msk: np.ndarray) -> dict:
    ...
    for i in np.unique(msk)[1:]:
        ...
        if m.sum() > 0:
            ...
            c = cv2.findContours(
                m.astype(np.uint8), mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_NONE
            )
...
def discard_small_submsks(
    msk: np.ndarray, pix_dict: dict = None, min_n_pix: int = 100
) -> np.ndarray:
...
def append_shapes_to_msk(msk: np.ndarray, shapes: list, start_idx: int) -> np.ndarray:
...
def get_submsks1_submsks2_status(
    msk1: np.ndarray,
    msk2: np.ndarray,
    ...
    prims = {"neg": prim_sec_neg, "pos": prim_sec_pos, "amb": prim_sec_amb}

    secs = {"neg": sec_prim_neg, "pos": sec_prim_pos, "amb": sec_prim_amb}
    ...
    def add_area(status_dict, submsks_pix_dict):
        ...
        for i in ["pos", "amb"]:
            ...
            for j in status_dict[i].keys():
                ...
                status_dict[i][j] = {
                    k: status_dict[i][j][k]
                    for k in ["status", "area", "summary", "data"]
                }

        status_dict["neg"] = dict(
            sorted(
                status_dict["neg"].items(), key=lambda x: (x[1]["area"]), reverse=True
            )
        )

        return

    add_area(status_dict=prims, submsks_pix_dict=submsks1_pix_dict)

    add_area(status_dict=secs, submsks_pix_dict=submsks2_pix_dict)
...
def convert_zvi_to_ome(
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(
            f"Image conversion failed (rc={proc.returncode}).\n"
            f"STDOUT:\n{proc.stdout}\n"
            f"STDERR:\n{proc.stderr}"
        )
...
def load_ome_tiff(file: str) -> tuple:
    ...
    for i, j in enumerate(planes):
        stage_pos[i] = {"position_x": j.position_x, "position_y": j.position_y}
...
def txt_to_elem_dfs(file: str) -> list:
    ...
    df = pd.read_csv(filepath_or_buffer=file, sep="\t", skiprows=7)
...
def scale_elem_cnt(
    ...
    stage_pos = [
        metadata_dict["image"]["position_x"],
        metadata_dict["image"]["position_y"],
    ]

    phys_size = [
        metadata_dict["image"]["physical_size_x"],
    ...
    if objective not in [10, 20]:
        ...
        raise NotImplementedError(
            f"Contour scaling not implemented for {objective}X objective yet."
        )
...
def format_elem_cnt(
    ...
    cnt = np.apply_along_axis(
        func1d=lambda x: ",".join(x.astype(str)), axis=1, arr=elem_cnt
    )
    ...
    cnt = pd.DataFrame(
        cnt, columns=["Color", "Thickness", "No", "Laser function", "CutShot"]
    )
...
def choose_stardist_n_tiles(
    img: np.ndarray, max_tile_px: int = 1024
) -> tuple[int, int]:
    """
...
def pickle_data(data: Any, filename: str | Path) -> None:
